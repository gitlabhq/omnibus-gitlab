  ...
  module Geo
    class ReplicationToggleCommand
      def initialize(ctl, action, args)
        ...
        @replication_process = GitlabCtl::Geo::ReplicationProcess.new(@ctl, @options)
      end

      def execute!
        process_action
        process_pitr_file
      rescue GitlabCtl::Geo::PitrFileError => e
      ...
      rescue GitlabCtl::Geo::PsqlError => e
        puts "Postgres encountered an error: #{e.message}"
        exit 1
      rescue GitlabCtl::Geo::RakeError => e
        puts "Rake encountered an error: #{e.message}"
        exit 1
      end

      def process_pitr_file
        geo_pitr_file = GitlabCtl::Geo::PitrFile.new(ctl)
