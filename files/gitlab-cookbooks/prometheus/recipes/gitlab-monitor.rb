...
gitlab_monitor_dir = node['prometheus']['gitlab-monitor']['home']
gitlab_monitor_log_dir = node['prometheus']['gitlab-monitor']['log_directory']
...
template "#{gitlab_monitor_dir}/gitlab-monitor.yml" do
  ...
  variables(
    probe_sidekiq: node['prometheus']['gitlab-monitor']['probe_sidekiq'],
    redis_url: redis_url,
...
runit_service "gitlab-monitor" do
  ...
  log_options node['gitlab']['logging'].to_hash.merge(node['prometheus']['gitlab-monitor'].to_hash)
end
