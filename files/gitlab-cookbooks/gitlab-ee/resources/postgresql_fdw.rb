...
action :delete do
  postgresql_query "drop fdw #{new_resource.server_name} on #{new_resource.db_name}" do
    ...
    not_if { new_resource.pg_helper.is_offline_or_readonly? || !new_resource.pg_helper.fdw_server_exists?(new_resource.server_name, new_resource.db_name) }
  end
  ...
  postgresql_query "drop postgres_fdw extension on #{new_resource.db_name}" do
    ...
    not_if do
      new_resource.pg_helper.is_offline_or_readonly? ||
        !new_resource.pg_helper.extension_enabled?('postgres_fdw', new_resource.db_name)
    end
  end
end
