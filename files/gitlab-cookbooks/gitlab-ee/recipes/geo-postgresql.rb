...
if node['gitlab']['geo-postgresql']['enable']
  ...
  bash 'refresh foreign table definition' do
    ...
    not_if { !gitlab_geo_helper.geo_database_configured? || pg_helper.is_managed_and_offline? || pg_helper.database_empty?(fdw_dbname) || geo_pg_helper.is_offline_or_readonly? || gitlab_geo_helper.fdw_synced? }
  end
end
