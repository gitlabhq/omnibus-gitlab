...
action :create do
  postgresql_query "create #{new_resource.schema} schema on #{new_resource.database}" do
    ...
    not_if do
      new_resource.pg_helper.is_offline_or_readonly? ||
        new_resource.pg_helper.schema_exists?(new_resource.schema, new_resource.database)
    end
  end
  ...
  postgresql_query "modify #{new_resource.schema} schema owner on #{new_resource.database}" do
    ...
    not_if do
      new_resource.pg_helper.is_offline_or_readonly? ||
        !new_resource.pg_helper.schema_exists?(new_resource.schema, new_resource.database) ||
        new_resource.pg_helper.schema_owner?(new_resource.schema, new_resource.database, new_resource.owner)
    end
  end
end
