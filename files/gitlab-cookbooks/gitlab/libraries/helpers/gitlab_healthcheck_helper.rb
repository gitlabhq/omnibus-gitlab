...
class GitlabHealthcheckHelper < BaseHelper
  ...
  def schema
    ...
    listen_https = node['gitlab']['gitlab_rails']['nginx']['listen_https']
  ...
  def host
    if nginx_enabled?
      "localhost:#{node['gitlab']['gitlab_rails']['nginx']['listen_port']}"
  ...
  def nginx_flags
    ...
    flags << '--haproxy-protocol' if node['gitlab']['gitlab_rails']['nginx']['proxy_protocol']
    flags << '--insecure'
    flags
  end
