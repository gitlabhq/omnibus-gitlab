...
> [!note]
> `udp_log_shipping_host`を設定すると、指定されたホスト名とサービスに対して、[`svlogd_prefix`が追加](https://gitlab.com/gitlab-org/omnibus-gitlab/blob/master/files/gitlab-cookbooks/gitlab/libraries/logging.rb)され、[runitによって管理される](../development/architecture/_index.md#runit)各サービスに適用されます。
...
> [!note]
> PostgreSQLは、外部プラグインなしではJSONロギングをサポートしていません。しかし、CSV形式でのロギングはサポートしています:
...
> [!note]
> ログ形式の属性名には、関連するサービスに応じていくつかのバリエーションがあります（たとえば、コンテナレジストリは`log_formatter`を使用し、GitalyとPraefectは両方とも`logging_format`を使用します）。詳細については、[Issue #4280](https://gitlab.com/gitlab-org/omnibus-gitlab/-/issues/4280)を参照してください。
...
> [!note]
> 一部のGitLabログ（たとえば`production_json.log`、`graphql_json.log`など）では、`log_level`を[編集することはできません](https://gitlab.com/groups/gitlab-org/-/work_items/6034)。また、[デフォルトのログレベルをオーバーライド](https://docs.gitlab.com/administration/logs/#override-default-log-level)も参照してください。
