...
- `log` indicates a [svlogd logging process](https://manpages.ubuntu.com/manpages/noble/man8/svlogd.8.html) attached to the preceding process.
...
You must wait for the `hup` command to finish. This could take some time. Leave the node out of the pool and do not restart services on the node where this is invoked until this completes. You also cannot use a Puma reload to update the Ruby runtime.
