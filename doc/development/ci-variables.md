...
| `USE_GO_FIPS_MODULE`           | Set to `true` to build Go components against upstream Go's native FIPS 140-3 module (`GOFIPS140`). FIPS builds imply it, so it is only needed to request the module on its own. |
| `GO_FIPS_MODULE_VERSION`       | Overrides the Go Cryptographic Module (`GOFIPS140`) version used when `Build::Check.use_go_fips_module?` is true. Defaults to the version pinned in `Build::Check::GO_FIPS_MODULE_VERSION`. |
