...
RSpec.describe 'Patroni' do
  ...
  describe '.parse_options' do
    before do
      allow(GitlabCtl::Patroni::Utils).to receive(:warn_and_exit).and_call_original
      allow(Kernel).to receive(:exit) { |code| raise "Kernel.exit(#{code})" }
      allow(Kernel).to receive(:warn)
    end

    it 'should throw error when global options are invalid' do
      expect { GitlabCtl::Patroni.parse_options(%w(patroni --foo)) }.to raise_error(OptionParser::ParseError)
    end

    it 'should throw error when sub-command is not specified' do
      expect { GitlabCtl::Patroni.parse_options(%w(patroni -v)) }.to raise_error(OptionParser::ParseError)
    end

    it 'should throw error when sub-command is not defined' do
      expect { GitlabCtl::Patroni.parse_options(%w(patroni -v foo)) }.to raise_error(OptionParser::ParseError)
    end

    it 'should recognize global options' do
      expect(GitlabCtl::Patroni.parse_options(%w(patroni -v -q members))).to include(quiet: true, verbose: true)
    end
    ...
    context 'when help option is passed' do
      it 'should show help message and exit for global help option' do
        expect { GitlabCtl::Patroni.parse_options(%w(patroni -h)) }.to raise_error('Kernel.exit(0)')
        expect(GitlabCtl::Patroni::Utils).to have_received(:warn_and_exit).with(/Usage help/)
      end

      all_commands.each do |cmd|
        it "should show help message and exit for #{cmd} help option" do
          expect { GitlabCtl::Patroni.parse_options(%W(patroni #{cmd} -h)) }.to raise_error('Kernel.exit(0)')
          expect(GitlabCtl::Patroni::Utils).to have_received(:warn_and_exit).with(instance_of(OptionParser))
        end
      end
    end
  end
  ...
  describe 'additional commands' do
    ...
    additional_commands.each do |cmd|
      it "should run the relevant patronictl command for #{cmd}" do
        GitlabCtl::Patroni.send(cmd.to_sym, command_options[cmd] || {})
  ...
  describe 'command output on non-Patroni node' do
    ...
    additional_commands.each do |cmd|
      it "should raise errors when running command #{cmd}" do
        expect { GitlabCtl::Patroni.send(cmd.to_sym, command_options[cmd]) }.to(
  ...
  describe '.init_db' do
    ...
    it 'should call initdb command with the specified options' do
      GitlabCtl::Patroni.init_db(command_options['bootstrap'])
  ...
  describe '.copy_config' do
    ...
    it 'should call initdb command with the specified options' do
      GitlabCtl::Patroni.copy_config(command_options['bootstrap'])
  ...
  describe '.leader?, .replica?, and .standyLeader?' do
    before do
      ...
      allow_any_instance_of(GitlabCtl::Patroni::Client).to receive(:get).with('/leader').and_yield(Struct.new(:code).new(leader_status))
      allow_any_instance_of(GitlabCtl::Patroni::Client).to receive(:get).with('/replica').and_yield(Struct.new(:code).new(replica_status))
      allow_any_instance_of(GitlabCtl::Patroni::Client).to receive(:get).with('/standby-leader').and_yield(Struct.new(:code).new(standby_leader_status))
    end
    ...
    context 'when node is leader' do
      ...
      it 'should identify node role' do
        expect(GitlabCtl::Patroni.leader?({})).to be(true)
        expect(GitlabCtl::Patroni.replica?({})).to be(false)
        expect(GitlabCtl::Patroni.standby_leader?({})).to be(false)
      end
    end
    ...
    context 'when node is replica' do
      ...
      it 'should identify node role' do
        expect(GitlabCtl::Patroni.leader?({})).to be(false)
        expect(GitlabCtl::Patroni.replica?({})).to be(true)
        expect(GitlabCtl::Patroni.standby_leader?({})).to be(false)
      end
    end
    ...
    context 'when node is standby leader' do
      ...
      it 'should identify node role' do
        expect(GitlabCtl::Patroni.leader?({})).to be(false)
        expect(GitlabCtl::Patroni.replica?({})).to be(false)
        expect(GitlabCtl::Patroni.standby_leader?({})).to be(true)
      end
    end
  end
  ...
  describe '.reinitialize_replica' do
    ...
    context 'when member option is set' do
      it 'calls reinit command with the specified options to reinitialize the cluster member' do
        GitlabCtl::Patroni.reinitialize_replica(command_options['reinitialize-replica'])
    ...
    context 'when member option is not set' do
      it 'calls reinit command with the specified options to reinitialize the current cluster member' do
        GitlabCtl::Patroni.reinitialize_replica(command_options['reinitialize-replica'].slice(:wait))
