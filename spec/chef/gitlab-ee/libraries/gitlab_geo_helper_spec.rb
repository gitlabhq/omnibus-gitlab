...
describe GitlabGeoHelper do
  ...
  describe '#fdw_synced?' do
    let(:state_file) { '/tmp/geo-fdw-sync-xyz-123' }

    before do
      allow(subject).to receive(:fdw_sync_status_file) { state_file }
    end

    it 'returns true when sync state file exists and content is zero' do
      stub_file_content(state_file, '0')

      expect(subject.fdw_synced?).to eq true
    end

    it 'returns false when sync state file doesnt exist' do
      allow(File).to receive(:exist?).with(state_file) { false }

      expect(subject.fdw_synced?).to eq false
    end
  ...
  describe '#fdw_sync_status_file' do
    let(:connection_digest) { '32596e8376077c3ef8d5cf52f15279ba' }
    let(:revision) { '8b21d70' }
    ...
    it 'contains connection digest' do
      expect(subject.fdw_sync_status_file).to include(connection_digest)
    end

    it 'contains revision number' do
      expect(subject.fdw_sync_status_file).to include(revision)
    end

    it 'follows specific pattern' do
      expect(subject.fdw_sync_status_file).to include("geo-fdw-sync-#{connection_digest}-#{revision}")
    end
  end
