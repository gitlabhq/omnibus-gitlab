...
RSpec.describe 'gitaly' do
  ...
  context 'with Omnibus gitconfig' do
    ...
    context 'with changed default value' do
      ...
      it "does not populate deprecated gitaly['gitconfig'] setting but use the new configuration option" do
        expect(chef_run.node['gitaly']['configuration']['git']['config']).to eq([{ "key" => "receive.fsckObjects", "value" => "false" }])
        expect(chef_run.node['gitaly']['gitconfig']).to be nil
      end
    end
